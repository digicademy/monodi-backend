    ...
    module Strategy
      class RsyncWithRemoteCache < Remote
        ...
        default_attribute :repository_cache, 'cached-copy'
	      default_attribute :deploy_subdir, ''
        ...
        def rsync_command_for(server)
          "rsync #{rsync_options} --rsh='ssh -p #{ssh_port(server)} #{rsync_ssh_options}' #{local_cache_path}/#{deploy_subdir} #{rsync_host(server)}:#{repository_cache_path}/"
        end
